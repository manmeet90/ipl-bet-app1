...
async function seed() {
  try {
    ...
    const existingMatches = await firebaseDB.getAllMatches();
    if (existingMatches.length > 0) {
      console.log(`Database already has ${existingMatches.length} matches. Skipping match seeding.`);
    } else {
    ...
    const existingAdmin = await firebaseDB.getUserByPhone('Admin');
    if (!existingAdmin) {
